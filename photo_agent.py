...
Bọc PhotoMasterEngineV2.process() trong vòng lặp:
...
PhotoMasterEngineV2.process() đã đọc từ `options` (face_restore_fidelity,
...
# PhotoMasterEngineV2.process). Nếu văn bản lỗi đổi bên đó, nhớ đồng bộ ở đây.
_FIXABLE_PREFIXES = (
    "Ảnh mờ",
...
@dataclass
class AgentResult:
    """Bọc thêm quanh Dict trả về từ PhotoMasterEngineV2.process()."""
...
class PhotoQAAgent:
    """Bọc PhotoMasterEngineV2 để tự retry khi ảnh chưa đạt chuẩn.
    ...
    def _escalate(options: Dict) -> Dict:
        ...
        key mà PhotoMasterEngineV2.process() thực sự đọc (xem photo_engine.py)."""
