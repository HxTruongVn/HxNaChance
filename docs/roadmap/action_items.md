...
> thật trong cấu trúc hiện tại (`app/`, `config/`, `workshops/layout/`,
> `setup/`, `ui/`, `workshops/photo/`, `api/`, entry point
> `NaChance.py`) — không copy
...
- **P0 #1+#2: `weights_only=True`** ở `workshops/photo/processors/face_parser.py`
  và `face_restorer.py` — thêm `_torch_load_safe()` dùng chung (đặt ở
  `workshops/photo/utils.py`, nhận `torch` module qua tham số thay vì
...
| # | Việc | File | Ghi chú |
|---|---|---|---|
| 1 | `torch.load(..., weights_only=False)` → `weights_only=True` | `workshops/photo/processors/face_parser.py` | Từ PyTorch 2.6+, `weights_only=False` cho phép thực thi code tuỳ ý nếu file `.pth` bị thay. Nếu load lỗi vì checkpoint có object khác tensor thuần, dùng `torch.serialization.add_safe_globals([...])` khai đúng class thay vì lùi lại `False`. |
| 2 | Tương tự #1 | `workshops/photo/processors/face_restorer.py` | File này còn đọc `checkpoint["params_ema"]` sau load — kiểm tra logic parse không đổi sau khi bật `weights_only=True`. |
...
`workshops/photo/engine.py` (đang hardcode `str(wdir / "codeformer.pth")`
trực tiếp).
...
**File**: `workshops/photo/engine.py` (dòng ~184 upscale, ~201 face_restore
